...
def point_cloud_to_numpy(point_cloud):
    ...
    # pc_data2 = ros2_numpy.numpify(point_cloud)
    ...
    # Construct a numpy record datatype from PointCloud2 fields
    dtype_list = []
    ...
    # # Unpack RGB to separate r, g, b channels
    # r = (rgb >> 16) & 0x0000ff
    # g = (rgb >> 8) & 0x0000ff
    # b = (rgb) & 0x0000ff

    # Stack or concatenate x, y, z, r, g, b into a single numpy array
...
class RosOperator(Node):
    ...
    def model_inference(self):
        ...
        stats_list = []
        set_seed(1000)
        if type(self.args.ckpt_dir) is str:
            self.args.ckpt_dir = [self.args.ckpt_dir]
        for i in range(len(self.args.ckpt_dir)):
            policy = make_policy(self.args)
            ckpt_path = os.path.join(self.args.ckpt_dir[i], self.args.ckpt_name)
            state_dict = torch.load(ckpt_path)
            new_state_dict = {}
            for key, value in state_dict.items():
                if key in ["model.is_pad_head.weight", "model.is_pad_head.bias"]:
                    continue
                if self.args.next_action_num == 0 and key in ["model.input_proj_next_action.weight",
                ...
                if self.args.backbone.startswith("resnet") and key == "model.pos.weight":
                    continue
            ...
            stats_path = os.path.join(self.args.ckpt_dir[i], self.args.ckpt_stats_name)
        ...
        if self.args.use_instruction:
            tokenizer = BertTokenizer.from_pretrained(self.args.instruction_encoder_dir)
            encoder = BertModel.from_pretrained(self.args.instruction_encoder_dir)
        else:
            tokenizer = None
            encoder = None

        chunk_size = self.args.chunk_size
        joint_state0_zero = [[0, 0, 0, 0, 0, 0, 0.08] for _ in range(len(self.args.arm_joint_state_ctrl_topics))]
        joint_state1_zero = [[0, 0, 0, 0, 0, 0, 0.08] for _ in range(len(self.args.arm_joint_state_ctrl_topics))]

        # joint_state0 = [[0.0055820800000000005, 0.8687984200000001, 0.0, -0.015280944000000001, -0.028747712, -0.010169852, 0.08] for _ in range(len(self.args.arm_joint_state_ctrl_topics))]
        # joint_state1 = [[0.0055820800000000005, 0.8687984200000001, 0.0, -0.015280944000000001, -0.028747712, -0.010169852, 0.08] for _ in range(len(self.args.arm_joint_state_ctrl_topics))]
        # end_pose0 = [[0.2408368057645897, 0.00287136069042693, -0.0009838999821004113, -0.024625887251749915, 0.7530362776143732, 0.006795072511917707, 0.08] for _ in range(len(self.args.arm_end_pose_ctrl_topics))]
        # end_pose1 = [[0.2408368057645897, 0.00287136069042693, -0.0009838999821004113, -0.024625887251749915, 0.7530362776143732, 0.006795072511917707, 0.08] for _ in range(len(self.args.arm_end_pose_ctrl_topics))]
        joint_state0 = [[0, 0, 0, 0, 0.4, 0, 0.08] for _ in range(len(self.args.arm_joint_state_ctrl_topics))]
        joint_state1 = [[0, 0, 0, 0, 0.4, 0, 0.08] for _ in range(len(self.args.arm_joint_state_ctrl_topics))]
        end_pose0 = [[0.2315508867606016, -2.365701964007318e-06, 0.1086502962013508, -5.032693615049478e-06, 0.3140743375720001, -1.2025435587192976e-05, 0.08] for _ in range(len(self.args.arm_end_pose_ctrl_topics))]
        end_pose1 = [[0.2315508867606016, -2.365701964007318e-06, 0.1086502962013508, -5.032693615049478e-06, 0.3140743375720001, -1.2025435587192976e-05, 0.08] for _ in range(len(self.args.arm_end_pose_ctrl_topics))]
        ...
        with torch.inference_mode():
            while rclpy.ok():
                ...
                if self.args.temporal_agg:
                    state_dim = 0 + (self.args.arm_joint_state_dim * len(self.args.arm_joint_state_names) if self.args.use_arm_joint_state > 1 else 0) + \
                                    (self.args.arm_end_pose_dim * len(self.args.arm_end_pose_names) if self.args.use_arm_end_pose > 1 else 0) + \
                                    (self.args.robot_base_dim if self.args.use_robot_base > 1 else 0)
                    all_time_actions = np.zeros([self.args.max_publish_step, self.args.max_publish_step + self.args.chunk_size, state_dim])
                ...
                while t < self.args.max_publish_step and rclpy.ok():
                    ...
                    if self.args.next_action_num != 0:
                        if t == 0:
                            inference_thread = threading.Thread(target=inference_process,
                                                                args=(self.args, stats, t,
                                                                      self, policy, tokenizer, encoder))
                        ...
                        if t >= max_t:
                            if inference_thread is not None:
                                ...
                                t_start = t
                                max_t = t_start + self.args.pos_lookahead_step
                                if self.args.temporal_agg:
                                    all_time_actions[[t_start], t_start:t_start + self.args.chunk_size] = all_actions
                                is_new_action = True
                                inference_lock.release()
                                inference_thread = threading.Thread(target=inference_process,
                                                                    args=(self.args, stats, t,
                                                                          self, policy, tokenizer, encoder,
                                                                          all_actions[0][:self.args.pos_lookahead_step]))
                                inference_thread.start()
                        ...
                        if self.args.asynchronous_inference:
                            ...
                            if t >= max_t or self.args.pos_lookahead_step == 0 or t % self.args.pos_lookahead_step == 0:
                                inference_thread = threading.Thread(target=inference_process,
                                                                    args=(self.args, stats, t,
                                                                          self, policy, tokenizer, encoder))
                                inference_thread.start()
                    ...
                    if category is not None and category == 0:
                        ...
                        if self.args.use_arm_joint_state > 1:
                            ...
                            action_joint_state = post_process(stats, "joint_state", self.args.qpos_norm_mode, is_action=True, data=action_joint_state)
                        ...
                        if self.args.use_arm_end_pose > 1:
                            ...
                            if not self.args.use_arm_end_pose_incre:
                                action_end_pose = post_process(stats, "end_pose", self.args.qpos_norm_mode, is_action=True, data=action_end_pose)
                        ...
                        if self.args.use_robot_base > 1:
                            ...
                            action_robot_base = post_process(stats, "robot_base", self.args.qpos_norm_mode, is_action=True, data=action_robot_base)
...
def get_arguments():
    ...
    parser.add_argument('--camera_color_parent_frame_ids', action='store', type=str, help='camera_color_parent_frame_ids',
                        default=['camera_l_link', 'camera_r_link', 'camera_fisheye_l_link', 'camera_fisheye_r_link'],
                        required=False)
    ...
    parser.add_argument('--camera_color_config_topics', action='store', type=str, help='camera_color_config_topics',
                        default=['/camera_l/color/camera_info', '/camera_r/color/camera_info', '/camera_fisheye_l/color/camera_info', '/camera_fisheye_r/color/camera_info'],
                        required=False)
    ...
    parser.add_argument('--camera_depth_parent_frame_ids', action='store', type=str, help='camera_depth_parent_frame_ids',
                        default=['camera_l_link', 'camera_r_link'],
                        required=False)
    ...
    parser.add_argument('--camera_depth_config_topics', action='store', type=str, help='camera_depth_config_topics',
                        default=['/camera_l/aligned_depth_to_color/camera_info', '/camera_r/aligned_depth_to_color/camera_info'],
                        required=False)
    ...
    parser.add_argument('--arm_end_pose_topics', action='store', type=str, help='arm_end_pose_topics',
                        default=['/piper_FK_l/urdf_end_pose', '/piper_FK_r/urdf_end_pose'],
                        required=False)
    ...
    parser.add_argument('--arm_end_pose_ctrl_topics', action='store', type=str, help='arm_end_pose_ctrl_topics',
                        default=['/piper_IK_l/ctrl_end_pose', '/piper_IK_r/ctrl_end_pose'],
                        required=False)
