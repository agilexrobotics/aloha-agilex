...
def model_inference(args, ros_operator):
    ...
    joint_state0 = [[0, 0, 0, 0, 0.4, 0, 0.08] for _ in range(len(args.arm_joint_state_ctrl_topics))]
    joint_state1 = [[0, 0, 0, 0, 0.4, 0, 0.08] for _ in range(len(args.arm_joint_state_ctrl_topics))]
    end_pose0 = [[0.2315508867606016, -2.365701964007318e-06, 0.1086502962013508, -5.032693615049478e-06, 0.3140743375720001, -1.2025435587192976e-05, 0.08] for _ in range(len(args.arm_end_pose_ctrl_topics))]
    end_pose1 = [[0.2315508867606016, -2.365701964007318e-06, 0.1086502962013508, -5.032693615049478e-06, 0.3140743375720001, -1.2025435587192976e-05, 0.08] for _ in range(len(args.arm_end_pose_ctrl_topics))]
    ...
    if args.use_arm_end_pose > 1:
        ros_operator.arm_joint_state_ctrl_linear_interpolation_thread(joint_state0_zero, True, calc_step=True)
